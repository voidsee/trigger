...
class Dkr:
    def __init__(self, ip_ports=[['127.0.0.1', 80],]):
        ...
        self.imgs = { 'nginx:latest':{'ports':{'80/tcp':7080}, 'environment':None},
                'mysql:5.6':{'ports':{'3306/tcp':3307}, 'environment':["MYSQL_ROOT_PASSWORD=123456"]},}
        self.info = { 'nginx:latest':'phpinfo',
                'mysql:5.6':'weak password',}
    ...
    def run_ctnrs(self,):
        ...
        for img in self.imgs:
            ...
            for j in range(len(splited_ports[i])):
                splited_ports[i][j].append(self.info[img])
