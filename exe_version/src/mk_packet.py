...
class Packet:
    ...
    def reply(self, rule, iface="eth0", match=lambda p: p.show()):
        self.receive = sniff(iface=iface, filter=rule, prn=match)
        wrpcap("receive_pks.pcap", self.receive)

if __name__ == "__main__":
    ip_ports=[["192.168.123.129",80], ["192.168.123.27",8080], ["192.168.123.202",8280]]
    ips=["192.168.123.129", "192.168.123.32", "192.168.123.25", "192.168.123.26"] 
    rp = Packet(ips, ip_ports, show=True)
    n = input("input q, 1, 2: ")
    while len(n)!=0 and n[0]!="q":
        if n[0]=="1":
            #rp.reply("icmp or arp", "eno1", rp.arp_icmp)
            rp.reply("icmp or arp", "wlo1", rp.arp_icmp)
        else:
            #rp.reply("tcp or arp", "eno1", rp.arp_tcp)
